...
export function createDefaultStatistics(): PlayerStatistics {
  const createEmptyRow = (): LevelStats => ({
    gamesPlayed: 0,
    gamesWon: 0,
    gamesLost: 0,
    lettersFound: 0,
...
export class StorageService {
  private readonly USERS_KEY = "users_db";
  private readonly CURRENT_USER_KEY = "current_user";
  private readonly JSON_PATH = "../../build/data/users.json";
  ...
  public async initDatabase(): Promise<void> {
    ...
    try {
      console.log(
        `[StorageService] Завантаження базових даних користувачів з ${this.JSON_PATH}...`,
      );
      const response = await fetch(this.JSON_PATH);

      if (!response.ok) {
        throw new Error(
          `Не вдалося завантажити JSON файл: ${response.statusText}`,
        );
      }
  ...
  public updateCurrentUserStats(updatedStats: PlayerStatistics): void {
  ...
  public async getCategoryData(category: string): Promise<CategoryData> {
