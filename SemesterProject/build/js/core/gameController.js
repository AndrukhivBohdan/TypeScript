import { createDefaultStatistics } from "../services/storageService.js";
const SYMBOL_POOL = [
...
export class GameController {
    ...
    difficultySettings = {
        easy: {
            hides: 5,
            time: 5,
            cssClass: "hide-trigger text-red-600 font-black cursor-pointer bg-yellow-200 rounded px-0.5 animate-pulse",
        },
        medium: {
            hides: 10,
            time: 4,
            cssClass: "hide-trigger text-blue-800 font-bold cursor-pointer hover:bg-blue-100 transition-colors",
        },
        hard: {
            hides: 15,
            time: 2,
    ...
    initializeGameWorld(hidesCount, timeLimit) {
        ...
        console.log("GAME WORLD BUILT. Hides generated:", this.activeHidingMap.length);
    }
    generateHides(pool, count) {
        const hides = [];
        if (!pool.length)
            return hides;
        for (let i = 0; i < count; i++) {
            const randomNode = pool[Math.floor(Math.random() * pool.length)];
            if (!randomNode?.text || randomNode.text.length === 0)
                continue;
    ...
    prepareDataForRender(category) {
        const originalData = this.categoryCache[category];
        if (!this.isGameActive || !originalData)
            return originalData;
        const processedItems = originalData.items.map((item) => {
            const itemHides = this.activeHidingMap.filter((h) => h.locationId === category && h.itemId === item.id);
            const injectHidesIntoString = (text, fieldName) => {
                if (!text)
                    return text;
                const fieldHides = itemHides.filter((h) => h.field === fieldName);
                if (!fieldHides.length)
                    return text;
                const sortedHides = [...fieldHides].sort((a, b) => b.charPosition - a.charPosition);
                ...
                for (const hide of sortedHides) {
                    ...
                    const currentClass = this.difficultySettings[this.currentDifficulty].cssClass;
            ...
            return {
                ...
                shortDescription: injectHidesIntoString(item.shortDescription, "shortDescription"),
                extendedDescription: item.extendedDescription
                    ? {
                        mainUsage: injectHidesIntoString(item.extendedDescription.mainUsage, "mainUsage"),
                        permissions: injectHidesIntoString(item.extendedDescription.permissions, "permissions"),
                        dimensions: injectHidesIntoString(item.extendedDescription.dimensions, "dimensions"),
                    }
    ...
    getHidesForLocation(locationId) {
    ...
    startNewGame(difficulty) {
        ...
        const diffKey = this.currentDifficulty;
        if (!stats[diffKey]) {
    ...
    handleHideFound(locationId, itemId, field, pos, element) {
        if (!this.isGameActive || !locationId || !itemId || !field)
            return;
        const hideIndex = this.activeHidingMap.findIndex((h) => h.locationId === locationId &&
