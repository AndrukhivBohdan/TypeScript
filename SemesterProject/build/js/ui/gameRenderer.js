export class GameRenderer {
    ...
    initAuthEventListeners() {
        ...
        loginForm.addEventListener("submit", (e) => {
            ...
            if (res.success && res.user) {
                if (window.HeaderHUD) {
                    window.HeaderHUD.username = res.user.username;
                }
    ...
    initGameStartMenuListeners(onStart) {
    ...
    applyHides(text, itemId, fieldName, category, hides) {
        if (!text)
            return "";
        if (!hides || hides.length === 0)
            return text;
        const chars = text.split("");
        const itemFieldHides = hides.filter((h) => String(h.itemId) === String(itemId) &&
            h.field === fieldName &&
            h.locationId === category);
        const activeDifficulty = window.AppController?.currentDifficulty || "easy";
        const difficultySettings = window.AppController
            ?.difficultySettings || {
            easy: {
        ...
        const currentClass = difficultySettings[activeDifficulty]?.cssClass ||
            difficultySettings.easy.cssClass;
        for (const hide of itemFieldHides) {
            if (hide.charPosition < chars.length) {
                const randomSymbol = hide.randomSymbol || chars[hide.charPosition];
